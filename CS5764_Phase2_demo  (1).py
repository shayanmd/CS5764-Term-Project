...
#df = pd.read_csv("new-realtor-data-50K.csv")
df = pd.read_csv("new-realtor-data-50K-part2.csv")
...
my_app.layout = html.Div([
    ...
    html.Br(),
    dcc.Tabs(id='questions',children=[ dcc.Tab(label = 'Dimensionality Reduction Techniques',value = 't0'),
                                         dcc.Tab(label = 'Price analysis',value = 't1'),
...
# one layout for dataset explanation
...
def return_image(selected_option):
    if selected_option == 'option-2d':
        return html.Img(id='2d-image', src=f'assets/2DPCA.jpeg')
         #return html.Img(src=r'assets/2DPCA.png', alt='image')

        #html.Img(id='img', src='assets/image.png')
        #return html.Img(id='img', src='assets/2DPCA.png')
        #return 'assets/2DPCA'  # Relative path to assets folder
    elif selected_option == 'option-3d':
        return html.Img(id='3d-image', src=f'assets/3DPCA.jpeg')

        #return html.Img(src=r'assets/3DPCA.png', alt='image')

        #return html.Img(id='img', src='assets/3DPCA.png')
       #return 'assets/3DPCA'
...
def update_graph(graph_type):
    ...
    if graph_type == 'lineplot':
        fig = px.line(df, x=df.index, y='price', title=f'Lineplot showing trends in house prices')
    elif graph_type == 'histogram':
        fig = px.histogram(df, x='price', title='Histogram displaying distribution of price values')
        # fig.update_layout(
        ...
        fig = px.bar(df, x='state', y='price', color = 'status',barmode='stack', title='Stacked bar chart showing price and status for states')  # Example palette)

    elif graph_type == 'grouped_bar':
        fig = px.bar(df, x='state', y='price',color = 'status' , barmode='group', height=400, title='Grouped bar chart showing price and status for states' )

         #color_continuous_scale="Viridis")   #
         #fig.update_layout(
             # plot_bgcolor="rgba(255, 255, 255, 1)",  # Set background to white
             # paper_bgcolor="rgba(255, 255, 255, 1)",  # Set outer chart background to white
         #)
